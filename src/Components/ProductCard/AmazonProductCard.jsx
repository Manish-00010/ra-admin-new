...
const AmazonProductCard = ({ product, isListView = false, onRemoveFavorite }) => {
  const navigate = useNavigate();

  return (
    <Card className="h-100 shadow-sm product-card amazon-product-card">
      <div className="position-relative">
        ...
        <Badge color="primary" className="position-absolute bottom-0 start-0 m-2 store-badge">
          Amazon
        </Badge>
      </div>
      <CardBody className={isListView ? "d-flex align-items-center" : ""}>
        <div className={isListView ? "flex-grow-1" : ""}>
          ...
          <div className="mt-3 ">
            <div>
              <span className="h5 price-primary mb-0">${product.price}</span>
            ...
            <div className="d-flex gap-2  mt-4">
              <Button
                ...
                size="sm"
                className="w-100"
                onClick={() => navigate(`/Amazon/Products/${product.id}`)}
              >
                View Details
              </Button>
              <Button
                color="primary"
                size="sm"
                className="btn-amazon w-100"
                onClick={() => window.open(`https://amazon.com/dp/${product.id}`, '_blank')}
              >
                Buy on Amazon
              </Button>
            </div>
